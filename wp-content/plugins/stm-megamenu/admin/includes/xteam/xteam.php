...
class STM_Nav_Menu_Item_Custom_Fields {
	static $options = array(); //phpcs:ignore

	static function view_control( $field ) { //phpcs:ignore
	...
	static function setup() { //phpcs:ignore
	...
	static function get_fields_schema( $new_fields ) { //phpcs:ignore
		$schema = array();
	...
	static function get_menu_item_postmeta_key( $name ) { //phpcs:ignore
		return '_menu_item_' . $name;
	}

	static function get_field( $item, $depth, $args ) { //phpcs:ignore
		$new_fields = '';
		foreach ( self::$options['fields'] as $field ) {

			if ( 'stm_mega_text_repeater' === $field['name'] ) {
			...
			if ( 'stm_menu_logo' === $field['name'] && get_post_meta( $item->ID, self::get_menu_item_postmeta_key( $field['name'] ), true ) === 'checked' ) {
				$field['checked'] = 'checked';
			}
			if ( 'stm_menu_show_special_offer' === $field['name'] && get_post_meta( $item->ID, self::get_menu_item_postmeta_key( $field['name'] ), true ) === 'checked' ) {
				$field['checked'] = 'checked';
			}
			if ( 'stm_menu_as_title' === $field['name'] && get_post_meta( $item->ID, self::get_menu_item_postmeta_key( $field['name'] ), true ) === 'checked' ) {
				$field['checked'] = 'checked';
			}
			if ( 'stm_child_menu_use_post' === $field['name'] && get_post_meta( $item->ID, self::get_menu_item_postmeta_key( $field['name'] ), true ) === 'checked' ) {
				$field['checked'] = 'checked';
			}
			$field['id'] = $item->ID;

			$view = self::view_control( $field );
			if ( ! empty( $field['options'] ) ) unset( $field['options'] ); //phpcs:ignore

			$new_fields .= str_replace(
				array_map( function ( $key ) { //phpcs:ignore
					return '{' . $key . '}';
				}, array_keys( $field ) ), //phpcs:ignore
	...
	static function _save_post( $post_id, $post ) { //phpcs:ignore
		...
		foreach ( self::$options['fields'] as $field_schema ) {
			...
			if ( 'menu-item-stm_mega_text_repeater' === $form_field_name && isset( $_POST['menu-item-stm_menu_icon_repeater'][ $post_id ] ) ) {
				if ( '' !== $_POST['menu-item-stm_menu_icon_repeater'][ $post_id ][0] ) {
					...
					$value = stripslashes( json_encode( $_POST['menu-item-stm_menu_icon_repeater'][ $post_id ], JSON_UNESCAPED_UNICODE ) ); //phpcs:ignore
					update_post_meta( $post_id, $key, $value );

					$key   = self::get_menu_item_postmeta_key( 'stm_menu_text_repeater' );
					$value = stripslashes( json_encode( $_POST['menu-item-stm_mega_text_repeater'][ $post_id ], JSON_UNESCAPED_UNICODE ) ); //phpcs:ignore
					update_post_meta( $post_id, $key, $value );
				} else {
					$key = self::get_menu_item_postmeta_key( 'stm_menu_icon_repeater' );
...
class STM_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit { //phpcs:ignore
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) { //phpcs:ignore
		$item_output = '';

		parent::start_el( $item_output, $item, $depth, $args );

		if ( $new_fields = STM_Nav_Menu_Item_Custom_Fields::get_field( $item, $depth, $args ) ) { //phpcs:ignore
